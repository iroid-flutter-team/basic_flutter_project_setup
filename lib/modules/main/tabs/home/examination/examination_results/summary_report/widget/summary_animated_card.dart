...
class SummaryAnimatedCard extends GetView<SummaryReportController> {
  final SummaryReportModel summaryReportModel;
  const SummaryAnimatedCard({Key? key, required this.summaryReportModel}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      ...
      return _buildAnimationSlider(
          summaryReportModel.localImagePathList.length, _infiniteCardsController);
    });
  }
  ...
  _buildAnimationSlider(
    ...
    _infiniteCardsController = InfiniteCardsController(
      ...
      itemBuilder: (BuildContext context, int index) {
        return summaryReportModel.questionSubmitted
            ? _buildNetworkPhotoView(index, _infiniteCardsController)
            : summaryReportModel.localImagePathList.isEmpty
            ? _buildTakePhotoView1()
            : _buildTakePhotoView2(
            imagePath: summaryReportModel.localImagePathList[index],
            index: index,
            infiniteCardsController: _infiniteCardsController!);
      },
  ...
  int _getItemCount() {
    ...
    if (summaryReportModel.questionSubmitted) {
      totalItem = summaryReportModel.imagePathList.length;
    } else {
      totalItem = summaryReportModel.localImagePathList.isEmpty
          ? 1
          : summaryReportModel.localImagePathList.length;
    }
    ...
    return CommonContainerWithShadow(
      ...
      child: GestureDetector(
        onTap: () {
          Get.to(
            ViewPhotoScreen(
              networkImagePath: summaryReportModel.imagePathList[index],
            ),
          );
        },
        onPanUpdate: (details) {
        ...
        child: Container(
          decoration: BoxDecoration(
            image: DecorationImage(
              fit: BoxFit.fill,
              image: NetworkImage(
                summaryReportModel.imagePathList[index],
              ),
            ),
        ...
        child: ClipRRect(
          ...
          child: Stack(
            children: <Widget>[
              ...
              summaryReportModel.localImagePathList.length < 3
                  ? Container(
                height: getSize(140),
      ...
      if (result != null) {
        if (result['action'] == 'ADD') {
          ...
          if (imagePath.isEmpty) {
            summaryReportModel.localImagePathList.insert(0, result['imagePath']);
          } else {
            summaryReportModel.localImagePathList.isNotEmpty
                ? summaryReportModel.localImagePathList
                .removeAt(index) //removes the item at index
                : null;
            summaryReportModel.localImagePathList.insert(
              index,
              result['imagePath'],
            );
          }
...
// class SummaryAnimatedCard extends GetView<SummaryReportController> {
//   const SummaryAnimatedCard({Key? key,}) : super(key: key);
//
//   @override
//   Widget build(BuildContext context) {
//     InfiniteCardsController? _infiniteCardsController;
