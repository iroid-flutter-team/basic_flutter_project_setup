...
class AnimatedCard extends GetView<QuestionController> {
  ...
  int _getItemCount() {
    ...
    if (questionModel.questionSubmitted.value) {
  ...
  _buildTakePhotoView1() {
    return GestureDetector(
      onTap: () async{
        AppFocus.unFocus();
        bool isCameraPermissionGranted = await ImagePickerUtils().checkCameraPermission();
        bool isMicrophonePermissionGranted = await ImagePickerUtils().checkMicrophonePermission();
...
//   }
//
//   _buildAnimationSlider(int imageCount,
//       InfiniteCardsController? _infiniteCardsController) {
//     _infiniteCardsController = InfiniteCardsController(
//       itemCount: _getItemCount(),
//       itemBuilder: (BuildContext context, int index) {
//         printInfo(info: 'TTTMaitry${questionModel.answer!.map((e) => e.toJson())}');
...
//   }
//
//   Widget _buildNetworkPhotoView(int index,
//       InfiniteCardsController? _infiniteCardsController) {
//     return CommonContainerWithShadow(
//       width: double.maxFinite,
//       child: GestureDetector(
...
//               fit: BoxFit.fill,
//               image: FileImage(
//                 File( questionModel.answer![index].toString(),)
//
//               ),
//             ),
//             borderRadius: BorderRadius.all(
...
//   }
//
//   _buildTakePhotoView2({required String imagePath,
//     required int index,
//     required InfiniteCardsController infiniteCardsController}) {
//     printInfo(info: 'imagePath == $imagePath');
//
...
//             answer.answerId = 0;
//             answer.image = result['imagePath'];
//             printInfo(
//                 info: 'answer list size = ${questionModel.answer?.length}');
//             questionModel.answer!.insert(0, answer);
//             printInfo(
//                 info: 'answer list size = ${questionModel.answer?.map((e) => e.toJson())}');
//           } else {
//             questionModel.answer!.isNotEmpty
//                 ? questionModel.answer!.removeAt(
...
//             answer.answerId = 0;
//             answer.image = result['imagePath'];
//             questionModel.answer!.insert(index, answer);
...
//       }
//     } catch (e) {
//       printInfo(info: 'Exception = ${e.toString()}');
//     }
//   }
// }
