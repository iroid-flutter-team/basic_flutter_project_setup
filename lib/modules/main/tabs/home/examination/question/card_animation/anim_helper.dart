...
class AnimHelper {
  ...
  late int _positionToBack, _positionToFront;
  ...
  void _addRemoveAnimEnd() {
    try {
      for (AnimationController controller
          in _animationControllerAddRemoveList) {
        controller.reset();
      }
      if (animCallback != null) {
  ...
  double _getCurveValue(double fraction) {
    if (controller.curve.flipped == null) {
      return fraction;
    } else {
      return controller.curve.transform(fraction);
    }
  }
}
